...
namespace RoAxisDimensionRemover
{
    ...
    /// Skopiowane 1:1 z wzorca w `Radius Dimention Mover` (ten sam katalog
    /// nadrzędny, ten sam mechanizm) - patrz tam po uzasadnienie każdej
    /// decyzji, jeśli trzeba coś zmienić.
    ///
    ...
    /// tagu release (`v0.1.0` → `0.1.0`). Te dwie wartości plus `MyAppVersion`
    /// w `installer/setup.iss` muszą być zgodne.
    /// </summary>
    internal static class UpdateCheck
    {
        private const string LatestReleaseApi =
            "https://api.github.com/repos/HoldFort-Bananza/RO-Axis-Dimension-Remover/releases/latest";

        public const string ReleasesPage =
            "https://github.com/HoldFort-Bananza/RO-Axis-Dimension-Remover/releases";

        private const int TimeoutMs = 5000;

        /// <summary>
        /// Odpala sprawdzanie w tle. `onNewerVersion` jest wołane TYLKO gdy
        /// znaleziono nowszą wersję - dostaje jej numer (np. "0.2.0"). Wołane
        /// z wątku roboczego, więc odbiorca musi sam wrócić na wątek UI.
        ///
        ...
        private static string FetchLatestTag()
        {
            try
            {
                ...
                request.UserAgent = "RoAxisDimensionRemover";
        ...
        /// zarówno "v0.1.0" jak i "0.1.0" są rozumiane. Tagi nienumeryczne dają
        /// null i są ignorowane.
        /// </summary>
