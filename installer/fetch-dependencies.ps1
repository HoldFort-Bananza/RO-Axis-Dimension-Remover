...
.SYNOPSIS
    ...
    zaleznosci wymagane przez RoAxisDimensionRemover.exe, i kopiuje je do
...
.DESCRIPTION
    ...
    Radius Dimention Mover (ten sam katalog nadrzedny) - zweryfikowane
    poprzez lokalny cache NuGet (~/.nuget/packages) uzyty przy budowaniu tego
    programu, bo obie aplikacje uzywaja tych samych API Tekla.Structures /
    .Drawing / .Model 2025.0.0 i dostaja te same zaleznosci przechodnie.

.PARAMETER TargetDir
    Folder, do ktorego maja trafic pobrane pliki .dll (folder instalacji
    RoAxisDimensionRemover.exe).
#>
param(
...
$tempDir = Join-Path $env:TEMP ("RoAxisDimensionRemover_deps_" + [System.Guid]::NewGuid().ToString("N"))
